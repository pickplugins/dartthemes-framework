...
class Kirki_Control_Repeater extends Kirki_Control_Base {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->row_label = array(
			'type'  => 'text',
			'value' => esc_attr__( 'row', 'dart-framework' ),
			'field' => false,
		);
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'dart-framework' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_attr__( 'Select a Page', 'dart-framework' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'dart-framework' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
						...
						<div class="repeater-field repeater-field-{{{ field.type }}}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<label>
									<# if ( field.label ) { #><span class="customize-control-title">{{{ field.label }}}</span><# } #>
									<# if ( field.description ) { #><span class="description customize-control-description">{{{ field.description }}}</span><# } #>
									<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'dart-framework' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'dart-framework' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_attr_e( 'No Image Selected', 'dart-framework' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'dart-framework' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'dart-framework' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'dart-framework' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change Image', 'dart-framework' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add Image', 'dart-framework' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'dart-framework' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_attr_e( 'No File Selected', 'dart-framework' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'dart-framework' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'dart-framework' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change File', 'dart-framework' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add File', 'dart-framework' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'dart-framework' ); ?></button>
				</div>
			</li>
		</script>
		<?php
