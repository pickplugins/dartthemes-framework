		...
		<div class="wrapper">
			...
			<# if ( data.default['font-family'] ) { #>
				...
				<div class="font-family">
					<h5><?php esc_attr_e( 'Font Family', 'dart-framework' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-font-family-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'dart-framework' ); ?>"></select>
				</div>
				<# if ( ! _.isUndefined( data.choices['font-backup'] ) && true === data.choices['font-backup'] ) { #>
					<div class="font-backup hide-on-standard-fonts kirki-font-backup-wrapper">
						<h5><?php esc_attr_e( 'Backup Font', 'dart-framework' ); ?></h5>
						<select {{{ data.inputAttrs }}} id="kirki-typography-font-backup-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'dart-framework' ); ?>"></select>
					</div>
				<# } #>
				<# if ( true === data.show_variants || false !== data.default.variant ) { #>
					<div class="variant kirki-variant-wrapper">
						<h5><?php esc_attr_e( 'Variant', 'dart-framework' ); ?></h5>
				...
				<# if ( true === data.show_subsets ) { #>
					<div class="subsets hide-on-standard-fonts kirki-subsets-wrapper">
						<h5><?php esc_attr_e( 'Subset(s)', 'dart-framework' ); ?></h5>
			...
			<# if ( data.default['font-size'] ) { #>
				...
				<div class="font-size">
					<h5><?php esc_attr_e( 'Font Size', 'dart-framework' ); ?></h5>
			...
			<# if ( data.default['line-height'] ) { #>
				...
				<div class="line-height">
					<h5><?php esc_attr_e( 'Line Height', 'dart-framework' ); ?></h5>
			...
			<# if ( data.default['letter-spacing'] ) { #>
				...
				<div class="letter-spacing">
					<h5><?php esc_attr_e( 'Letter Spacing', 'dart-framework' ); ?></h5>
			...
			<# if ( data.default['word-spacing'] ) { #>
				...
				<div class="word-spacing">
					<h5><?php esc_attr_e( 'Word Spacing', 'dart-framework' ); ?></h5>
			...
			<# if ( data.default['text-align'] ) { #>
				...
				<div class="text-align">
					<h5><?php esc_attr_e( 'Text Align', 'dart-framework' ); ?></h5>
					<div class="text-align-choices">
						<input {{{ data.inputAttrs }}} type="radio" value="inherit" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-inherit" <# if ( data.value['text-align'] === 'inherit' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-inherit">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Inherit', 'dart-framework' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="left" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-left" <# if ( data.value['text-align'] === 'left' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-left">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Left', 'dart-framework' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="center" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-center" <# if ( data.value['text-align'] === 'center' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-center">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Center', 'dart-framework' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="right" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-right" <# if ( data.value['text-align'] === 'right' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-right">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Right', 'dart-framework' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="justify" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-justify" <# if ( data.value['text-align'] === 'justify' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-justify">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Justify', 'dart-framework' ); ?></span>
							</label>
						</input>
					</div>
				</div>
			...
			<# if ( data.default['text-transform'] ) { #>
				...
				<div class="text-transform">
					<h5><?php esc_attr_e( 'Text Transform', 'dart-framework' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-transform-{{{ data.id }}}">
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'dart-framework' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Capitalize', 'dart-framework' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Uppercase', 'dart-framework' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Lowercase', 'dart-framework' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Initial', 'dart-framework' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'dart-framework' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( data.default['text-decoration'] ) { #>
				...
				<div class="text-decoration">
					<h5><?php esc_attr_e( 'Text Decoration', 'dart-framework' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-decoration-{{{ data.id }}}">
						<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'dart-framework' ); ?></option>
						<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Underline', 'dart-framework' ); ?></option>
						<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Overline', 'dart-framework' ); ?></option>
						<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Line-Through', 'dart-framework' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Initial', 'dart-framework' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'dart-framework' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( data.default['margin-top'] ) { #>
				...
				<div class="margin-top">
					<h5><?php esc_attr_e( 'Margin Top', 'dart-framework' ); ?></h5>
			...
			<# if ( data.default['margin-bottom'] ) { #>
				...
				<div class="margin-bottom">
					<h5><?php esc_attr_e( 'Margin Bottom', 'dart-framework' ); ?></h5>
			...
			<# if ( false !== data.default['color'] && data.default['color'] ) { #>
				...
				<div class="color">
					<h5><?php esc_attr_e( 'Color', 'dart-framework' ); ?></h5>
